...
LGraphSDFShader.title = "OutputSDF";
LGraphSDFShader.desc = "Output Main SDF Node";


LGraphSDFShader.prototype.setValue = function(v)
{

};

LGraphSDFShader.prototype.onExecute = function()
{
    this.processInputCode();

}

LGraphSDFShader.prototype.onDrawBackground = function(ctx)
{
    //show the current value
    //this.outputs[0].label = this.properties["value"].toFixed(3);
}

LGraphSDFShader.prototype.onWidget = function(e,widget)
{

}

LGraphSDFShader.prototype.sortPathByOrder = function (map)
{
    var tupleArray = [];
    for (var key in map) tupleArray.push([key, map[key]]);
    tupleArray.sort(function (a, b) { return a[1].order - b[1].order });
...
LGraphSDFShader.prototype.getFullCode = function(slot, scope, modifier) {
    CodePiece.ORDER_MODIFIER = modifier;
    var path = this.getInputNodePath(slot);
...
LGraphSDFShader.prototype.processInputCode = function() {

    var color_code = null;
    var normal_code = null;
   
    var emission_code = null;
    var specular_code = null;
    var gloss_code = null;
    var alpha_code = null;
    var alphaclip_code = null;
    var refraction_code = null;
    var world_offset_code =null;
     
    var sdfProc = this.getFullCode(0, CodePiece.FRAGMENT,0);
    var sdfMaterialProc = this.getFullCode(1, CodePiece.FRAGMENT,0);
    var shader = this.shader_piece.createShader(this.graph.scene_properties ,color_code,normal_code,emission_code,specular_code,gloss_code,alpha_code,alphaclip_code,refraction_code, world_offset_code,sdfProc,sdfMaterialProc);
...
LiteGraph.registerNodeType("core/"+ LGraphSDFShader.title ,LGraphSDFShader);
