...
vik.app = (function () {
    ...
    function loadContent() {
        ...
        var cameraInside = new EZ.ECamera(45, w / h, 0.1, 1000);
        cameraInside.position = [0, 2, 2];
        cameraInside.target = [0,1,0];

        var scene = new EZ.EScene();
        main_node.mesh = "plane";
        main_node.flags.blend = false;
        main_node.shader = "phong";
        main_node.position = [0, 0.5, 0];
        main_node.scale = [5,5,5];
        ...
        module.cameraInside = cameraInside;
        module.camera = camera;
        module.resolution = {x:w,y:h};
        ...
        function render() {
            ...
            renderer.render(scene, camera,cameraInside);
    ...
    function addEnvironmentGlobals(shader) {
        ...
        shader.globals["camera_pos"]  = {name:"u_camera_pos", value: module.camera.position , getValue:function(){return [this.value[0],this.value[1],this.value[2]]}};
        shader.globals["camera_target"]  = {name:"u_camera_target", value: module.camera.target , getValue:function(){return [this.value[0],this.value[1],this.value[2]]}};
        shader.globals["resolution"]  = {name:"iResolution", value: module.resolution , getValue:function(){
    ...
    module.resize = function () {
        ...
        if ((w > 0 || h > 0) && (w != renderer.context.canvas.width || h != renderer.context.canvas.height)) {
            renderer.resize(w, h);
            module.resolution.x = w;
            module.resolution.y = h;
        }
        vik.ui.onResize();
    }
